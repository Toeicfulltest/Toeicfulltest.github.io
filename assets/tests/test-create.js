...
export function createTestCreateController(ctx){
  ...
  function kindCards(cls="test-kind"){
    return Object.entries(TEST_KINDS).map(([key,cfg])=>`<button type="button" class="card ${cls} ${key==="reading"?"active":""}" data-kind="${key}" role="radio" aria-checked="${key==="reading"?"true":"false"}"><b>${cfg.label}</b><span class="muted">Mặc định ${cfg.duration} phút</span></button>`).join("");
  }
  ...
  async function open(){
    ...
    modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal wide create-test-modal">
      ...
      <div class="tabs create-mode-tabs"><button type="button" class="btn tab active create-mode" data-mode="manual">Tạo thủ công</button><button type="button" class="btn tab create-mode" data-mode="import">Nhập từ file</button><button type="button" class="btn tab create-mode" data-mode="clone">Từ bài kiểm tra cũ</button><button type="button" class="btn tab create-mode" data-mode="bank">Tạo từ ngân hàng</button></div>
      ...
      <section class="create-mode-panel" data-mode-panel="bank" hidden><div class="test-kind-grid" role="radiogroup" aria-label="Loại bài từ ngân hàng">${kindCards("bank-test-kind")}</div><form id="bankTestForm" class="form-grid"><input type="hidden" name="test_kind" value="reading"><label class="span-2">Tên bài<input name="title" required placeholder="Tên bài kiểm tra mới"></label><label class="span-2">Lớp áp dụng<select name="class_ids" multiple size="5">${options}</select>${classHint}</label><label>Thời gian (phút)<input type="number" name="duration_minutes" value="75" min="1" required></label><label>Số lần làm<input type="number" name="max_attempts" value="1" min="1" required></label><label>Chống gian lận<select name="anti_cheat_mode"><option value="warn_then_submit">Cảnh báo rồi tự nộp</option><option value="strict">Nghiêm ngặt</option><option value="off">Tắt</option></select></label><label>Mở lúc<input type="datetime-local" name="opens_at"></label><label>Đóng lúc<input type="datetime-local" name="closes_at"></label><label class="check-row span-2"><input type="checkbox" name="show_answers_after_submit" checked> Cho xem đáp án sau khi nộp</label><div class="span-2 info-box bank-kind-note">${kindNote("reading")}</div><div class="span-2 warning-box"><b>Ngân hàng chỉ dùng mục “Đã duyệt”.</b> Hệ thống tạo bài nháp rồi mở ngay màn hình chọn câu. Với Listening/Full Test, cấu trúc câu có thể lấy từ bank nhưng bài thi hiện vẫn cần <b>1 file audio chung Part 1–4</b> trong Cài đặt.</div><button type="submit" class="primary span-2">Tạo bài và chọn từ ngân hàng</button></form></section>
    </div></div>`;
    if(!migrationReady){modalRoot.querySelectorAll(".test-kind,.bank-test-kind").forEach(b=>{if(b.dataset.kind!=="reading")b.disabled=true;});}
    bindModes();bindManual(migrationReady);bindImport(migrationReady);bindClone();bindBank(migrationReady);
  }
  ...
  function bindBank(migrationReady){
    const form=modalRoot.querySelector("#bankTestForm"),kind=form.elements.test_kind,duration=form.elements.duration_minutes,note=modalRoot.querySelector(".bank-kind-note");
    const selectKind=value=>{const cfg=testKindConfig(value);kind.value=value;duration.value=cfg.duration;note.innerHTML=kindNote(value);modalRoot.querySelectorAll(".bank-test-kind").forEach(b=>{const active=b.dataset.kind===value;b.classList.toggle("active",active);b.setAttribute("aria-checked",String(active));});};
    modalRoot.querySelectorAll(".bank-test-kind").forEach(b=>b.onclick=()=>selectKind(b.dataset.kind));
    form.onsubmit=async e=>{e.preventDefault();const fd=new FormData(form),f=Object.fromEntries(fd);if(f.test_kind!=="reading"&&!migrationReady)return toast("Hãy chạy migration V1.18 trước khi tạo Listening/Full Test.",7000);f.class_ids=fd.getAll("class_ids");f.duration_minutes=Number(f.duration_minutes);f.max_attempts=Number(f.max_attempts);f.opens_at=toIso(f.opens_at);f.closes_at=toIso(f.closes_at);Object.assign(f,{status:"draft",description:"",show_answers_after_submit:fd.has("show_answers_after_submit"),allowed_violations:2});const btn=e.submitter||form.querySelector('button[type="submit"]');btn.disabled=true;btn.textContent="Đang tạo…";try{const {data,error}=await sb.rpc("staff_upsert_test_v120",{p_data:f});if(error)throw error;const id=typeof data==="string"?data:(data?.id||data?.test_id);if(!id)throw new Error("Backend không trả về mã bài kiểm tra.");await createPartsForKind(sb,id,f.test_kind);sessionStorage.setItem("toeic.openBankGenerator",id);invalidate();closeModal();toast(`Đã tạo ${testKindLabel(f.test_kind)} · đang mở ngân hàng`);go(`/test/${id}/authoring`);}catch(err){console.error(err);toast(`Không tạo được bài từ ngân hàng: ${err.message||err}`,7000);btn.disabled=false;btn.textContent="Tạo bài và chọn từ ngân hàng";}};
  }
